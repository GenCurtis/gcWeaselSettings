...
- **配色**：Catppuccin Mocha（自定义 `preset_color_schemes/mocha`，accent = Mauve）
...
| `weasel.custom.yaml` | 外观配置（配色 / 字体 / 字号） |
| `default.custom.yaml` | 全局配置（方案 / 候选数） |
...
4. **部署（静默）**：停 WeaselServer → `WeaselDeployer.exe /deploy`（headless 写 build、不弹窗）→ 重启 server。一次性应用全部配置（mocha/字体/字号/候选数/方案）。
...
3. **静默部署（不弹窗）**：`/deploy` 时若 WeaselServer 在跑，server 进 maintenance 模式会弹窗。要完全静默：**停 server → `/deploy`（headless 写 build）→ 重启 server**（`setup.ps1` 内置此流程）。`/deploy` 一次性应用全部配置（mocha/字体/字号/候选数/方案），无回退、无需删 build。
...
- 配色源自 [Catppuccin](https://github.com/catppuccin/catppuccin) · Mocha
